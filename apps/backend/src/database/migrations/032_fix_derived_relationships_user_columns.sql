-- Migration: Fix derived relationships functions to use correct user columns
-- Created: 2024-12-07
-- Description: Fixes the enhanced derived relationships functions to use display_name instead of first_name/last_name

BEGIN;

-- Step 1: Fix get_swap_with_derived_relationships function
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(su.display_name, 'Unknown User') as proposer_name,
    ...
    COALESCE(tu.display_name, 'Unknown User') as target_owner_name,
...
-- Step 2: Fix get_multiple_swaps_with_derived_relationships function
...
-- Step 3: Fix get_swap_relationships_summary function
...
-- Step 4: Test the corrected functions
CREATE OR REPLACE FUNCTION test_corrected_derived_relationships_functions()
...
BEGIN
  ...
  BEGIN
    ...
    RETURN QUERY
    SELECT 
      'get_swap_with_derived_relationships_corrected'::text,
      TRUE,
  ...
  BEGIN
    ...
    RETURN QUERY
    SELECT 
      'get_multiple_swaps_with_derived_relationships_corrected'::text,
      TRUE,
      CONCAT('Function executed successfully and returned ', function_result_count, ' results')::text;
  EXCEPTION
    WHEN OTHERS THEN
  ...
  BEGIN
    ...
    RETURN QUERY
    SELECT 
      'get_swap_relationships_summary_corrected'::text,
      TRUE,
...
BEGIN
    RAISE NOTICE 'Running corrected derived relationships function validation...';
    
    FOR validation_result IN SELECT * FROM test_corrected_derived_relationships_functions() LOOP
    ...
    IF has_errors THEN
        RAISE WARNING 'Some corrected derived relationships function validation had failures, but continuing migration.';
    END IF;
    
    RAISE NOTICE 'Corrected derived relationships function validation completed.';
END;
$$;

-- Step 6: Add documentation comments
COMMENT ON FUNCTION get_swap_with_derived_relationships IS 'Corrected helper function to get swap with all derived relationships using display_name instead of first_name/last_name';
COMMENT ON FUNCTION get_multiple_swaps_with_derived_relationships IS 'Corrected helper function to get multiple swaps with derived relationships using display_name';
COMMENT ON FUNCTION get_swap_relationships_summary IS 'Corrected helper function to get a summary of swap relationships using display_name';
COMMENT ON FUNCTION test_corrected_derived_relationships_functions IS 'Validation function to test corrected derived relationships functions work correctly';

-- Final success message
DO $$
BEGIN
    RAISE NOTICE '=== Migration 032: Fixed Derived Relationships User Columns Completed Successfully ===';
    RAISE NOTICE 'Fixed function: get_swap_with_derived_relationships';
    RAISE NOTICE 'Fixed function: get_multiple_swaps_with_derived_relationships';
    RAISE NOTICE 'Fixed function: get_swap_relationships_summary';
    RAISE NOTICE 'Functions now use display_name instead of non-existent first_name/last_name columns';
END;
$$;
