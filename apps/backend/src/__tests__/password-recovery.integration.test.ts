...
// Mock both EmailService implementations
vi.mock('../services/email/EmailService', () => ({
  EmailService: vi.fn().mockImplementation(() => ({
    sendEmail: mockTransporter.sendMail,
    sendPasswordResetEmail: mockTransporter.sendMail,
    sendPasswordResetConfirmationEmail: mockTransporter.sendMail,
    verifyConnection: mockTransporter.verify,
  })),
}));

vi.mock('../services/notification/EmailService', () => ({
  EmailService: vi.fn().mockImplementation(() => ({
    sendEmail: mockTransporter.sendMail,
    verifyConnection: mockTransporter.verify,
  })),
...
describe('Password Recovery Integration Tests', () => {
  ...
  describe('Password Reset Request Flow', () => {
    it('should successfully initiate password reset for existing user', async () => {
      ...
      expect(emailCall.subject).toBe('Reset Your Password - Booking Swap Platform');
      expect(emailCall.html).toContain('Reset Password');
      expect(emailCall.html).toContain('https://example.com/reset-password?token=secure-reset-token');
    });
  ...
  describe('Password Reset Token Validation', () => {
    ...
    it('should reject expired reset token', async () => {
      // Mock expired token lookup (should return empty due to WHERE clause)
      mockPool.query.mockResolvedValueOnce({ rows: [] });

      const result = await authService.validateResetToken('expired-reset-token');
    ...
    it('should reject used reset token', async () => {
      // Mock used token lookup (should return empty due to WHERE clause)
      mockPool.query.mockResolvedValueOnce({ rows: [] });

      const result = await authService.validateResetToken('used-reset-token');
  ...
  describe('Email Service Integration Tests', () => {
    ...
    it('should send password reset email with correct template', async () => {
      ...
      expect(emailCall.html).toContain('https://example.com/reset?token=test-token');
      expect(emailCall.text).toContain('Test User');
      expect(emailCall.text).toContain('https://example.com/reset?token=test-token');
    });
